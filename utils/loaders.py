...
class EpicKitchensDataset(data.Dataset, ABC):
    ...
    def _get_train_indices(self, record, modality='RGB'):
        ...
        centroids = np.linspace(0, tot_frames, num_clip + 2)[1:-1].astype(int)

        frames = []
        for c in centroids:
            if tot_frames > 2 * num_clip * num_frames_per_clip:
                tmp = [c + j for j in range(-num_frames_per_clip, num_frames_per_clip, 2)]
            else:
                tmp = [c + j for j in range(-num_frames_per_clip // 2, num_frames_per_clip // 2)]
